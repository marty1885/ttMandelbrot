...
void MAIN {
    ...
    uint32_t start_row = get_arg_val<uint32_t>(6);
    uint32_t end_row = get_arg_val<uint32_t>(7);
    uint32_t tiles_per_row = width / (TILE_WIDTH * TILE_HEIGHT);
    ...
    for(uint32_t y = start_row; y < end_row; y++) {
        ...
        for(uint32_t x = 0; x < tiles_per_row; x += 1) {
            tile_regs_acquire();
            tile_regs_wait();
            float l = left + (right - left) * x / tiles_per_row;
            float r = l + (right - left) / tiles_per_row;
